...
/* USB AUDIO device Configuration Descgitriptor */
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[USB_AUDIO_CONFIG_DESC_SIZ] __ALIGN_END =
{
		  ...
		  /* 09 byte*/
			  ...
			  (AUDIO_INTERFACE_DESC_SIZE+1),        /* bLength */ //plus one because additional baInterfaceNr
			  AUDIO_INTERFACE_DESCRIPTOR_TYPE,      /* bDescriptorType */
			  AUDIO_CONTROL_HEADER,                 /* bDescriptorSubtype */
			  0x00,          /* 1.00 */             /* bcdADC */
			  0x01,
			  0x28,                               	/* wTotalLength = 49*/
			  0x00,
			  0x02,                                 /* bInCollection */
			  0x01,                                 /* baInterfaceNr */
			  0x02,                                 /* baInterfaceNr */
...
static uint8_t USBD_AUDIO_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  /* Open EP IN */
  USBD_LL_OpenEP(pdev, AUDIO_IN_EP, USBD_EP_TYPE_ISOC, AUDIO_IN_SIZE);
...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  UNUSED(pdev);
  UNUSED(epnum);
  if(epnum != (AUDIO_IN_EP & 0x0F)) return (uint8_t)USBD_OK;
  __disable_irq();
...
static uint8_t USBD_AUDIO_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  uint16_t PacketSize;
  USBD_AUDIO_HandleTypeDef *haudio;
  if(epnum != (AUDIO_OUT_EP & 0x0F)) return (uint8_t)USBD_OK;
  haudio = (USBD_AUDIO_HandleTypeDef *)pdev->pClassData;
...
void USBD_AUDIO_Rerun(void)
{
	USBD_HandleTypeDef *pdev = GetUsbDeviceFS();
